...
const Promotions = () => {
  ...
  const toastTest = () => {
    toast(
      "Thank you for claim! You will receive a email with coupon code. CODE: ASDAZC",
      {
        duration: 10000,
  ...
  const claim = (item) => {
    if (user.points >= item.points) {
      // Scadem puncte
      axios
        .post(`${HOST}:${PORT}/substractPoints`, {
          uid: user.uid,
          points: item.points,
        })
                ...
                <div class="xl:w-1/3 md:w-1/2 p-4" onClick={() => claim(item)}>
                  <div class="bg-white p-6 rounded-lg">
                    <img
                      ...
                      src="https://media.londontheatredirect.com/voucher/email-images/generic-voucher.png"
                    ...
                    <p class="leading-relaxed text-base">
                      Offered by: {item.oferitde}
                    </p>
                    <a class="btn-sm text-gray-200 bg-gray-900 hover:bg-gray-800 mt-4">
                      <span>Claim Now</span>
                      <svg
